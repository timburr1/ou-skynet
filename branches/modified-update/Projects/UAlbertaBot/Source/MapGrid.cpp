...
MapGrid * MapGrid::getInstance() 
{
	...
	if (!MapGrid::instance) 
	{
		MapGrid::instance = new MapGrid(BWAPI::Broodwar->mapWidth()*32, BWAPI::Broodwar->mapHeight()*32, 320);
	}
...
void MapGrid::update() 
{
	...
	{
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(i*cellSize, 0, i*cellSize, mapHeight, BWAPI::Colors::Blue);
	}

	for (int j=0; j<rows; j++) 
	{
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(0, j*cellSize, mapWidth, j*cellSize, BWAPI::Colors::Blue);
	}
	...
	for (int r=0; r < rows; ++r)
	{
		for (int c=0; c < cols; ++c)
		{
			...
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(cell.center.x(), cell.center.y(), "Last Seen %d", cell.timeLastVisited);
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(cell.center.x(), cell.center.y()+10, "Row/Col (%d, %d)", r, c);
		}
	}
