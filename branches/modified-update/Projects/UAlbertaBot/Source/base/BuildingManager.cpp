...
BuildingManager::BuildingManager() 
	: debugMode(false), reservedMinerals(0), reservedGas(0), buildingSpace(1), totalBuildTasks(0)
{
...
void BuildingManager::update() 
{
	...
	//BuildingPlacer::getInstance()->drawReservedTiles();
...
void BuildingManager::assignWorkersToUnassignedBuildings() 
{
	...
	while (buildingData.hasNextBuilding(ConstructionData::Unassigned)) 
	{
		...
		BWAPI::Unit * workerToAssign = WorkerManager::getInstance()->getBuilder(b);
		...
		if (workerToAssign) {
			...
			// reserve this space
			BuildingPlacer::getInstance()->reserveTiles(b.finalPosition, b.type.tileWidth(), b.type.tileHeight());
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
	...
	{
		return BuildingPlacer::getInstance()->getRefineryPosition();
	}

	// special case for early pylons
	if (b.type == BWAPI::UnitTypes::Protoss_Pylon && (numPylons < 3))
	{
		BWAPI::TilePosition posInRegion =    BuildingPlacer::getInstance()->getBuildLocationNear(b, 4, true);
		BWAPI::TilePosition posNotInRegion = BuildingPlacer::getInstance()->getBuildLocationNear(b, 4, false);
	...
	else
	{
		...
		{
			// get the location 
			BWAPI::TilePosition tile = MapTools::getInstance()->getNextExpansion();
		...
		else
		{
			...
			BWAPI::TilePosition posInRegion =    BuildingPlacer::getInstance()->getBuildLocationNear(b, distance, true,  horizontalOnly);

			// get a region anywhere
			BWAPI::TilePosition posNotInRegion = BuildingPlacer::getInstance()->getBuildLocationNear(b, distance, false, horizontalOnly);
	...
	while(buildingData.hasNextBuilding(ConstructionData::Assigned)) 
	{
		...
		if (!b.builderUnit->isConstructing()) 
		{
			...
			else if (b.buildCommandGiven) 
			{
				...
				WorkerManager::getInstance()->finishedWithWorker(b.builderUnit);

				// free the previous location in reserved
				BuildingPlacer::getInstance()->freeTiles(b.finalPosition, b.type.tileWidth(), b.type.tileHeight());
...
void BuildingManager::drawBuildingInformation(int x, int y) {
	...
	{
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(unit->getPosition().x(), unit->getPosition().y()+5, "\x07%d", unit->getID()); 
	}

	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y, "\x04 Building Information:");
	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y+20, "\x04 Name");
	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x+150, y+20, "\x04 State");
	...
	while (buildingData.hasNextBuilding(ConstructionData::Unassigned)) {
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y+40+((yspace)*10), "\x03 %s", b.type.getName().c_str());
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x+150, y+40+((yspace++)*10), "\x03 Need %c", getBuildingWorkerCode(b));
	}
	...
	while (buildingData.hasNextBuilding(ConstructionData::Assigned)) {
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y+40+((yspace)*10), "\x03 %s %d", b.type.getName().c_str(), b.builderUnit->getID());
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x+150, y+40+((yspace++)*10), "\x03 A %c (%d,%d)", getBuildingWorkerCode(b), b.finalPosition.x(), b.finalPosition.y());
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(b.builderUnit->getPosition().x(), b.builderUnit->getPosition().y(), (x1+x2)/2, (y1+y2)/2, BWAPI::Colors::Orange);
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawBoxMap(x1, y1, x2, y2, BWAPI::Colors::Red, false);
	}
	...
	while (buildingData.hasNextBuilding(ConstructionData::UnderConstruction)) {
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y+40+((yspace)*10), "\x03 %s %d", b.type.getName().c_str(), b.buildingUnit->getID());
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x+150, y+40+((yspace++)*10), "\x03 Const %c", getBuildingWorkerCode(b));
	}
}

BuildingManager * BuildingManager::instance = NULL;
