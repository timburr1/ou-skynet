...
void MicroManager::execute(const SquadOrder & inputOrder, BWAPI::Position regroup)
{
	...
	{
		MapGrid::getInstance()->GetUnits(nearbyEnemies, order.position, 800, false, true);
	...
	else if (order.type == order.Attack) 
	{
		...
		BOOST_FOREACH (BWAPI::Unit * unit, units) 
		{
			...
			MapGrid::getInstance()->GetUnits(nearbyEnemies, unit->getPosition(), 800, false, true);
		}
	}
	...
	if (order.type == order.Attack || order.type == order.Defend) {
		...
		executeMicro(nearbyEnemies, regroup);
	}	
}
...
bool MicroManager::unitNearEnemy(BWAPI::Unit * unit)
{
	...
	MapGrid::getInstance()->GetUnits(enemyNear, unit->getPosition(), 800, false, true);
...
void MicroManager::smartAttackUnit(BWAPI::Unit * attacker, BWAPI::Unit * target)
{
	...
	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(	attacker->getPosition().x(), attacker->getPosition().y(),
									target->getPosition().x(), target->getPosition().y(),
									BWAPI::Colors::Red );

}

void MicroManager::smartAttackMove(BWAPI::Unit * attacker, BWAPI::Position targetPosition)
{
	// if we have issued a command to this unit already this frame, ignore this one
...
void MicroManager::smartMove(BWAPI::Unit * attacker, BWAPI::Position targetPosition)
{
	// if we have issued a command to this unit already this frame, ignore this one
	if (attacker->getLastCommandFrame() >= BWAPI::Broodwar->getFrameCount())
	{
...
void MicroManager::drawOrderText() {
	...
	BOOST_FOREACH (BWAPI::Unit * unit, units) {
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(unit->getPosition().x(), unit->getPosition().y(), "%s", order.getOrderString().c_str());
	}
}
