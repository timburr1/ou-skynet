...
class ActionInProgress
{
	...
	friend class AIP_compare_desc;

	Action action;
	TimeType time;
	
	ActionInProgress() : action(0), time(0) {
		memset(this, 0, sizeof(*this));
	}
	ActionInProgress(unsigned char a, int t) : action(a), time(t) {}
};

class AIP_compare_desc { public: bool operator()(const ActionInProgress<unsigned short> & x,const ActionInProgress<unsigned short> & y) { return x.time > y.time; } };

class ActionsInProgress
{

	ActionInProgress<unsigned short>	inProgress[MAX_PROGRESS];
	unsigned char		inProgressSize;
	unsigned int 		numProgress[MAX_ACTIONS];	// how many of each unit are in progress
	
public:

	

	ActionsInProgress() : inProgressSize(0) { 
	...
	int numInProgress(Action a) const
	{
		return numProgress[a];	
	}
	...
	void addAction(Action a, int time)
	{
		...
		inProgress[inProgressSize] = ActionInProgress<unsigned short>(a, time);
		...
		if ((inProgressSize > 1) && (time > inProgress[inProgressSize-2].time))
		{
			...
			std::sort(inProgress, inProgress + inProgressSize, AIP_compare_desc());
		}
	}
	...
	unsigned int nextActionFinishTime() const
	{
		assert(inProgressSize > 0);
	...
	int nextActionFinishTime(Action a) const
	{
	...
	int whenActionsFinished(ActionSet actions) const
	{
