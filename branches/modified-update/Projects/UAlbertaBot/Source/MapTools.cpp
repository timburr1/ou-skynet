...
MapTools * MapTools::instance = NULL;
MapTools * MapTools::getInstance() 
{
	// if the instance doesn't exist, create it
	if (!MapTools::instance) 
	{
		MapTools::instance = new MapTools();
	}

	return MapTools::instance;
}

// constructor for MapTools
MapTools::MapTools() :	rows(BWAPI::Broodwar->mapHeight()), 
...
void MapTools::update()
{
	...
	if (nextExp != BWAPI::TilePositions::None)
	{
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawBoxMap(exp.x(), exp.y(), exp.x()+4*32, exp.y()+3*32, BWAPI::Colors::Green, true);
	}
	...
	/*for (int x=0; x<BWAPI::Broodwar->mapWidth(); ++x)
	{
		for (int y=0; y<BWAPI::Broodwar->mapHeight(); ++y)
		{
			...
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(p.x(), p.y(), "%d", getMyBaseDistance(p));
		}
	}*/
	...
	BOOST_FOREACH (BWAPI::Unit * unit, BWAPI::Broodwar->getAllUnits())
	{
		...
		if (unit->getPlayer() != BWAPI::Broodwar->self())
		{
			...
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(unit->getPosition().x(), unit->getPosition().y(), 3, c);
			//if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextMap(unit->getPosition().x(), unit->getPosition().y(), "%s", unit->getType().getName().c_str());
		}
	}
	...
	for (int i=0; i<BWAPI::Broodwar->mapWidth(); ++i)
	{
		for (int j=0; j<BWAPI::Broodwar->mapHeight(); ++j)
		{
			...
			if (BWTA::getRegion(tile) == startRegion)
			{
				...
				if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(i*32+16, j*32+16, 5, c);
			}
		}
	}
}
...
int MapTools::getEnemyBaseDistance(BWAPI::Position p)
{
	...
	if (!calculatedEnemyDistance)
	{
		...
		MapTools::getInstance()->computeDistance(enemyBaseMap, enemyBasePosition);
...
int MapTools::getMyBaseDistance(BWAPI::Position p)
{
	if (!calculatedMyDistance)
	{
		...
		MapTools::getInstance()->computeDistance(myBaseMap, myBasePosition);
...
BWAPI::TilePosition MapTools::getNextExpansion()
{
	...
	BOOST_FOREACH(BWTA::BaseLocation * base, BWTA::getBaseLocations())
	{
		...
		if(!base->isMineralOnly() && !(base == BWTA::getStartLocation(BWAPI::Broodwar->self())))
		{
			...
			double distanceFromHome = MapTools::getInstance()->getMyBaseDistance(BWAPI::Position(tile));//homeTile.getDistance(tile);
