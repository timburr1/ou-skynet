...
void ScoutManager::moveScouts()
{
	...
	BWTA::BaseLocation * enemyBaseLocation = UnitInfoState::getInstance()->getMainBaseLocation(BWAPI::Broodwar->enemy());
	...
	if (enemyRegion && scoutRegion)
	{
		...
		if (scoutRegion == enemyRegion)
		{
			...
			else
			{
				...
				if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(fleeTo.x(), fleeTo.y(), 10, BWAPI::Colors::Red);

				BOOST_FOREACH (BWAPI::Unit * unit, BWAPI::Broodwar->getUnitsInRadius(fleeTo, 10))
				{
					if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(unit->getPosition().x(), unit->getPosition().y(), 5, BWAPI::Colors::Cyan, true);
				}
...
BWAPI::Position ScoutManager::calcFleePosition(const std::vector<GroundThreat> & threats, BWAPI::Unit * target) 
{
	...
	while (r <= 360) 
	{
		...
		//if (drawDebugVectors) 
		//{
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(workerScout->getPosition().x(), workerScout->getPosition().y(), test.x(), test.y(), BWAPI::Colors::Cyan);
		//}
	...
	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawLineMap(workerScout->getPosition().x(), workerScout->getPosition().y(), fleeTo.x(), fleeTo.y(), BWAPI::Colors::Orange);
...
bool ScoutManager::isValidFleePosition(BWAPI::Position pos) 
{
	...
	// get the set of units that exist on the TilePosition (32*32) from bwapi
	...
	BWTA::BaseLocation * enemyLocation = UnitInfoState::getInstance()->getMainBaseLocation(BWAPI::Broodwar->enemy());

	BWAPI::Unit * geyser = getEnemyGeyser();
	if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(geyser->getPosition().x(), geyser->getPosition().y(), geyserDist, BWAPI::Colors::Red);
...
bool ScoutManager::immediateThreat()
{
	...
	BOOST_FOREACH(BWAPI::Unit * unit, BWAPI::Broodwar->enemy()->getUnits())
	{
		if (unit->getType().isWorker() && unit->isAttacking())
		{
			...
			if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(unit->getPosition().x(), unit->getPosition().y(), 5, BWAPI::Colors::Yellow);
		}
	}
