...
HardCodedInfo * HardCodedInfo::getInstance() 
{
...
BWAPI::Position HardCodedInfo::getChokepoint(ChokeType type, BWAPI::Player * player)
{
	...
	double closestDist = 1000; //reduce all by factor of 10
	BWAPI::Position closestPos(1000,1000);
...
void HardCodedInfo::drawChokePoints()
{
	...
	{
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawCircleMap(p.x(), p.y(), 10, BWAPI::Colors::Purple, true);
	}
