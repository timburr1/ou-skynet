...
void BuildOrderQueue::drawQueueInformation(int x, int y, int index) {

	//x = x + 25;
	
	std::string prefix = "\x04";

	//if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y, "\x04Priority Queue Information:");
	...
	for (size_t i(0); i<reps; i++) {
		...
		if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y+(i*10), "%s%s", prefix.c_str(), queue[queue.size() - 1 - i].metaType.getName().c_str());
	}
}
