...
public:
	...
	DFBBStarcraftSearch<StarcraftStateType>(SearchParameters<StarcraftStateType> p) : 
			params(p),
			winnerWorkerCount(0), 
			winner(true), 
			nodesExpanded(0), 
	...
	{		
		...
		if (s.meetsGoal(params.goal)) 
		{
			...
			if (finishTime < upperBound  || ((finishTime == upperBound) && (s.getWorkerCount() > winnerWorkerCount))) 
			{
				...
				results = SearchResults(true, upperBound, nodesExpanded, searchTimer.getElapsedTimeInMilliSec(), getBuildOrder(s));
		...
		{				
			...
			if (h <= upperBound)
			{
				...
				{
					child.setParent(&s);
					child.setActionPerformedK(repeat);
					DFBB(child, depth+1);
				}
			}
		}	
