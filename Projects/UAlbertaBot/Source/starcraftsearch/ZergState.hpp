...
public:
	...
	Hatchery(int n, int t) : numLarva(n), lastThirdUsed(t) {}
	Hatchery() : numLarva(0), lastThirdUsed(0) {}
};
...
public:
	...
	ZergState(bool init) 
	{
		...
		if (init) 
		{
			actionPerformed = -1; parent = NULL; currentFrame = 0;
	...
	{
		if (GSN_DEBUG) printf("\tSpecific Do Action\n");
	...
	virtual void specificFastForward(const int toFrame)
	{
		...
		{	
			...
			if (hatcheries[i].numLarva < 3) 
			{
				...
				int diff = toFrame - hatcheries[i].lastThirdUsed;

				// figure out how many larva should have spawned
				int numLarvaSpawn = (diff / ZERG_LARVA_TIMER);
	...
	{
		if (GSN_DEBUG) printf("\tSpecific Finish Action\n");
