...
public:
	...
	BuildingStatus(Action t, TimeType time) : type(t), timeRemaining(time) {}
};
...
class BuildingData
{
	...
	BuildingStatus<unsigned short> buildings[MAX_BUILDINGS];
...
public:
	...
	void addBuilding(const Action t)
	{
		...
		buildings[numBuildings++] = BuildingStatus<TimeType>(t, 0);
	}

	void addBuilding(const Action t, const int timeUntilFree)
	{
		assert(DATA[t].isBuilding());
		assert(numBuildings < (MAX_BUILDINGS - 1));
	
		buildings[numBuildings++] = BuildingStatus<TimeType>(t, timeUntilFree);
	}

	BuildingStatus<TimeType> & getBuilding(int i) const
	{
	...
	int timeUntilFree(const Action t) const
	{
		int min = 9999999;
	...
	{	
		...
		for (int i=0; i<numBuildings; ++i)
		{
			...
			{
				// queue it here
				buildings[i].timeRemaining = DATA[a].buildTime();
	...
	void fastForwardBuildings(const int frames)
	{
		for (int i=0; i<numBuildings; ++i)
		{
