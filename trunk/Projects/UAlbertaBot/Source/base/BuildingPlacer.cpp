	...
	for(int x = 0; x < rwidth; ++x) 
	{
		for(int y = 0; y < rheight; ++y) 
		{
			if (reserveMap[x][y] || isInResourceBox(x,y))
			{
				...
				if (DRAW_UALBERTABOT_DEBUG) BWAPI::Broodwar->drawBoxMap(x1, y1, x2, y2, BWAPI::Colors::Yellow, false);
			}
		}
	}
}
...
BuildingPlacer * BuildingPlacer::getInstance() {
	...
	if (!BuildingPlacer::instance) { 
		BuildingPlacer::instance = new BuildingPlacer(); 
	}
	return BuildingPlacer::instance;
}
