...
class StarcraftSearchData
{
	...
	SearchParameters<StarcraftStateType>			previousParameters;
	...
	SearchResults lastSolvedResults;
...
public:
	...
	StarcraftSearchData<StarcraftStateType>() :		searchInProgress(false), 
													lastSearchFrame(0), 
													searchFrames(0), 
													totalNodes(0),
													branch(0),
													totalSearchTime(0) {}
	~StarcraftSearchData<StarcraftStateType>() {}
	...
	void update(double timeLimit)
	{
		...
		drawSearchResults(10, 240);
	}

	void drawSearchResults(int x, int y)
	{
		if (DRAW_UALBERTABOT_DEBUG)
		{
	...
	void startNewSearch(SearchParameters<StarcraftStateType> params, bool forceNewSearch = false)
	{
		...
		if (forceNewSearch || !(previousParameters.goal == params.goal))
		{
			...
			lastSolvedResults = SearchResults();
